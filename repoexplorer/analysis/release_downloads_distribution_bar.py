...
def plot_release_downloads_distribution_bar(
    filtered_data,
    acronym="",
    ax=None,
    color_map=None,
    ...
    """
    Bar chart: count of repositories per release-downloads bucket (0-10, 11-100, ...).
    """
    ...
    if title_prefix:
        ax.set_title(
            rf"$\bf{{{title_prefix}\ {acronym}\ Release\ Downloads}}$ (Total: {total})",
            fontsize=title_size,
            loc="left",
            pad=18,
        )
        ...
        ax.set_title(
            rf"$\bf{{Release\ Downloads\ Distribution}}$ (Total: {total})",
            fontsize=title_size,
            loc="center",
            pad=18,
        )

    ax.set_xlabel("Release downloads (bucket)", fontsize=label_size)
    if not hide_ylabel:
...
def plot_release_downloads_distribution_bar_altair(
    filtered_data,
    acronym="",
    label_size=10,
    title_size=12,
    ...
    """Altair bar chart: count of repositories per release-downloads bucket."""
    width = "container"
    height = "container"
    ...
    if (
        ...
        return (
            ...
            .mark_bar()
            .properties(width=width, height=height, title="Release Downloads Distribution")
        )
    ...
    bars = (
        ...
        .encode(
            x=alt.X(
                "bucket:N",
                sort=_BUCKET_LABELS,
                title="Release downloads (bucket)",
    ...
    title = f"Release Downloads Distribution (Total: {total})"
    if acronym:

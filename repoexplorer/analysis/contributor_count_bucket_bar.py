...
def _values_to_bucket_labels(values: pd.Series) -> pd.Series:
    """
    Map contributor counts to buckets. NaN / negative -> NA.
...
def plot_contributor_count_bucket_bar(
    filtered_data,
    acronym="",
    ax=None,
    color_map=None,
    ...
    """
    Bar chart: repositories per contributor-count bucket; bar labels = % of bucketed rows.
    """
    ...
    if title_prefix:
        ...
        ax.set_title(
            rf"$\bf{{Contributor\ Count\ Distribution}}$ (Total: {total})",
            fontsize=title_size,
            loc="center",
            pad=18,
        )

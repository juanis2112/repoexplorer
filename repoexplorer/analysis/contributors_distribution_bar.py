...
def plot_contributors_distribution_bar(
    filtered_data,
    acronym="",
    ax=None,
    color_map=None,
    ...
    """
    Bar chart: count of repositories per contributor-count bucket (0-10, 11-100, ...).
    Uses column ``contributor_count``.
    """
    ...
    if title_prefix:
        ax.set_title(
            rf"$\bf{{{title_prefix}\ {acronym}\ Contributors}}$ (Total: {total})",
            fontsize=title_size,
            loc="left",
            pad=18,
        )
        ...
        ax.set_title(
            rf"$\bf{{Contributors\ Distribution}}$ (Total: {total})",
            fontsize=title_size,
            loc="center",
            pad=18,
        )

    ax.set_xlabel("Contributors (bucket)", fontsize=label_size)
    if not hide_ylabel:
...
def plot_contributors_distribution_bar_altair(
    filtered_data,
    acronym="",
    label_size=10,
    title_size=12,
    ...
    """Altair bar chart: count of repositories per contributor-count bucket."""
    width = "container"
    height = "container"
    ...
    title = f"Contributors Distribution (Total: {total})"
    if acronym:
